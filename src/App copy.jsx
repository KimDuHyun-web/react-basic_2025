...
function App() {
  ...
  */
  ...
  }, [id]); //최초 한 번 실행, id 가 변경되면 새롭게 함수 생성

  if(mode === 'welcome'){
    _title = welcome.title;
    _desc = welcome.desc;
    _article = <ReadArticle mode={mode} title={_title} desc={_desc}/>
  }else if(mode === 'read'){
    ...
    _article = <CreateArticle onsubmit={(_title, _desc)=>{
      ...
      setMaxId(newId);
      setId(newId);
      setMode('read'); // 입력한 내용을 바로 보려면 이거랑 바로 위에 꺼 작성하면 된다.
    }} />
  }else if(mode === 'update'){
    const selected = menus.find(menu=>menu.id === id);
    _title = selected.title;
